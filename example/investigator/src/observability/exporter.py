...
class JSONSpanExporter(SpanExporter):
    ...
    def __init__(self, output_dir: str = "traces", conversation_id: Optional[str] = None):
        ...
        try:
            for span in spans:
                ...
                if not self.conversation_id:
                    self.conversation_id = span_dict.get("attributes", {}).get("conversation_id", "unknown")
                
                # Track start time from first span
                if not self._start_time and span_dict.get("start_time"):
                    self._start_time = datetime.fromisoformat(span_dict["start_time"].replace("Z", "+00:00"))
    ...
    def _span_to_dict(self, span: ReadableSpan) -> Dict[str, Any]:
        ...
        end_time = datetime.fromtimestamp(span.end_time / 1e9).isoformat() + "Z" if span.end_time else None
    ...
    def _write_trace_file(self) -> None:
        ...
        timestamp_str = self._start_time.strftime("%Y%m%d_%H%M%S") if self._start_time else datetime.now().strftime("%Y%m%d_%H%M%S")
        filename = f"trace_{timestamp_str}_{short_id}.json"
        filepath = self.output_dir / filename
        
        # Calculate summary statistics
        total_duration_ms = sum(span.get("duration_ms", 0) or 0 for span in self._spans_buffer)
