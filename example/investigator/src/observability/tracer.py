...
class TracerSetup:
    ...
    def __init__(self, output_dir: str = "traces", service_name: str = "investigator-agent", conversation_id: Optional[str] = None):
    ...
    def initialize(self) -> trace.Tracer:
        ...
        self._exporter = JSONSpanExporter(output_dir=self.output_dir, conversation_id=self.conversation_id)
...
def get_tracer_setup(output_dir: str = "traces", service_name: str = "investigator-agent", conversation_id: Optional[str] = None) -> TracerSetup:
    ...
    if _tracer_setup is None:
        _tracer_setup = TracerSetup(output_dir=output_dir, service_name=service_name, conversation_id=conversation_id)
    return _tracer_setup


def initialize_tracing(output_dir: str = "traces", service_name: str = "investigator-agent", conversation_id: Optional[str] = None) -> trace.Tracer:
    ...
    Returns:
        Configured tracer instance
    """
    setup = get_tracer_setup(output_dir=output_dir, service_name=service_name, conversation_id=conversation_id)
