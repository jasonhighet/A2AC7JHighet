...
def test_normalize_tool_inputs_langsmith_format():
    """Test normalizing LangSmith trace input format."""
    # LangSmith wraps tool inputs in {'input': "{'key': 'value'}"}
    raw_inputs = {"input": "{'feature_id': 'FEAT-MS-001', 'analysis_type': 'metrics/unit_test_results'}"}

    result = _normalize_tool_inputs(raw_inputs)

    assert result == {"feature_id": "FEAT-MS-001", "analysis_type": "metrics/unit_test_results"}


def test_normalize_tool_inputs_empty_dict():
    """Test normalizing empty inputs."""
    raw_inputs = {"input": "{}"}
...
def test_normalize_tool_inputs_none():
    """Test normalizing None inputs."""
    result = _normalize_tool_inputs(None)

    assert result == {}


def test_normalize_tool_inputs_already_normalized():
    """Test inputs that don't need normalization."""
    # Direct dict format (used in unit tests)
    raw_inputs = {"feature_id": "FEAT-MS-001", "analysis_type": "metrics/unit_test_results"}
...
def test_normalize_tool_inputs_invalid_string():
    """Test handling invalid string format gracefully."""
    raw_inputs = {"input": "not a valid dict"}
...
def test_extract_feature_from_run_with_analysis_call():
    ...
    # Leaf nodes must have child_runs=None to stop recursion
    child_run = Mock()
    child_run.name = "get_analysis"
    child_run.run_type = "tool"  # Required for _extract_tool_calls to find it
    child_run.inputs = {"feature_id": "FEAT-MS-001", "analysis_type": "unit_test"}
    child_run.outputs = {}
    child_run.child_runs = None  # Leaf node
...
def test_extract_tool_calls():
    ...
    # Create mock child runs (leaf nodes must have child_runs=None)
    jira_call = Mock()
    jira_call.run_type = "tool"
    jira_call.name = "get_jira_data"
    jira_call.inputs = {}
    jira_call.outputs = {"features": []}
    jira_call.child_runs = None  # Leaf node
    ...
    analysis_call.child_runs = None  # Leaf node

    run = Mock()
    run.run_type = "chain"  # Parent is not a tool
...
def test_extract_tool_calls_deeply_nested():
    """Test extracting tool calls from deeply nested LangGraph structure.

    This mimics the real LangGraph trace structure:
    agent (root)
      └── ChatAnthropic
      └── should_continue
      └── tools
      ...
      └── agent
            └── ChatAnthropic
    """
    # Create leaf tool nodes
    jira_call = Mock()
    jira_call.run_type = "tool"
    jira_call.name = "get_jira_data"
    jira_call.inputs = {}
    jira_call.outputs = {"features": []}
    jira_call.child_runs = None

    analysis_call = Mock()
    analysis_call.run_type = "tool"
    analysis_call.name = "get_analysis"
    analysis_call.inputs = {"feature_id": "FEAT-CT-004", "analysis_type": "metrics/unit_test_results"}
    analysis_call.outputs = {}
    analysis_call.child_runs = None

    # Create "tools" node that contains tool calls
    tools_node = Mock()
    tools_node.run_type = "chain"
    tools_node.name = "tools"
    tools_node.child_runs = [jira_call, analysis_call]

    # Create other intermediate nodes
    chat_node = Mock()
    chat_node.run_type = "llm"
    chat_node.name = "ChatAnthropic"
    chat_node.child_runs = None

    should_continue = Mock()
    should_continue.run_type = "chain"
    should_continue.name = "should_continue"
    should_continue.child_runs = None

    # Create root agent node
    run = Mock()
    run.run_type = "chain"
    run.name = "agent"
    run.child_runs = [chat_node, should_continue, tools_node]
    ...
    assert result[0]["name"] == "get_jira_data"
    assert result[1]["name"] == "get_analysis"
    assert result[1]["inputs"]["feature_id"] == "FEAT-CT-004"


def test_extract_feature_from_run_deeply_nested():
    """Test extracting feature_id from deeply nested LangGraph structure."""
    # Create leaf get_analysis node
    analysis_call = Mock()
    analysis_call.name = "get_analysis"
    analysis_call.run_type = "tool"
    analysis_call.inputs = {"feature_id": "FEAT-CT-004", "analysis_type": "metrics/unit_test_results"}
    ...
    tools_node.name = "tools"
    tools_node.run_type = "chain"
    tools_node.child_runs = [analysis_call]

    # Create root agent node
    run = Mock()
    run.name = "agent"
    run.run_type = "chain"
    run.child_runs = [tools_node]

    result = _extract_feature_from_run(run)

    assert result == "FEAT-CT-004"


def test_extract_feature_from_run_tool_type_fallback():
    """Test extracting feature_id by run_type=tool when name doesn't match."""
    # Create a tool node where name might not be exactly "get_analysis"
    # (e.g., it might have a suffix or different format)
    analysis_call = Mock()
    analysis_call.name = "some_other_name"  # Not "get_analysis"
    analysis_call.run_type = "tool"  # But it IS a tool
    analysis_call.inputs = {"feature_id": "FEAT-RS-003", "analysis_type": "metrics/unit_test_results"}
    analysis_call.child_runs = None

    run = Mock()
    run.name = "agent"
    run.run_type = "chain"
    run.child_runs = [analysis_call]

    result = _extract_feature_from_run(run)

    assert result == "FEAT-RS-003"
...
def test_evaluate_feature_identification_correct():
    ...
    # Create mock run (leaf nodes must have child_runs=None)
    child_run = Mock()
    child_run.name = "get_analysis"
    child_run.run_type = "tool"  # Required for _extract_tool_calls to find it
...
def test_evaluate_tool_usage_all_correct():
    ...
    # Create mock tool calls (leaf nodes must have child_runs=None)
    jira_call = Mock()
    jira_call.run_type = "tool"
    jira_call.name = "get_jira_data"
    jira_call.inputs = {}
    ...
    analysis_call1.child_runs = None  # Leaf node
    ...
    analysis_call2.child_runs = None  # Leaf node
