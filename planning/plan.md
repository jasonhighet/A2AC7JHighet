...
- Multi-provider support (Anthropic, OpenRouter, LLMStudio, etc)
...
- Environment: **Native Windows** (No Docker, Containers, or WSL)
- Assistant: **Antigravity** (optimized for Windows/PowerShell)
- Use **PowerShell** for all terminal commands and script execution
- For local LLM inference, use **LLMStudio** instead of Ollama
- Clear, readable Python code that shows exactly what's happening
...
- When you're running tests and Python scripts, remember that the `python` binary is in the virtual environment (e.g. `.\.venv\Scripts\python.exe` on Windows)
...
- **LLMStudio** for local LLM inference (replacing Ollama)
