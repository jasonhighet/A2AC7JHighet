...
JSON data files from the data/incoming directory structure.
"""
...
def get_feature_folder_mapping() -> Dict[str, str]:
    ...
    This function scans the data/incoming directory and reads each feature's
    JIRA issue file to extract the feature_id (customfield_10001) and map it
    to the corresponding folder path.

    Returns:
        Dict mapping feature_id → folder path (relative to data/incoming/)
...
def get_analysis_file_path(folder_name: str, analysis_type: str) -> Path:
    ...
    Example:
        ...
        PosixPath('.../data/incoming/feature1/metrics/unit_test_results.json')
    """
