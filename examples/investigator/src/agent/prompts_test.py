...
def test_system_prompt_structure():
    ...
    # Should not be excessively long (reasonable length for comprehensive guidance)
    assert len(prompt) < 20000  # Updated for detailed investigation framework
...
def test_system_prompt_mentions_workflow_steps():
    ...
    assert "assess" in prompt.lower() or "analysis" in prompt.lower()
    assert "recommend" in prompt.lower() or "provide" in prompt.lower() or "decision" in prompt.lower()
...
def test_system_prompt_contains_workflow_guidance():
    ...
    # Should contain workflow or framework section
    assert "workflow" in prompt.lower() or "framework" in prompt.lower()

    # Should mention calling get_jira_data first
    assert "call `get_jira_data()`" in prompt.lower() or "get_jira_data()" in prompt.lower()
